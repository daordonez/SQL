SELECT numero,ciclista
FROM `etapes`
GROUP BY ciclista
ORDER BY ciclista DESC;
SELECT ciclista
FROM `etapes`
GROUP BY ciclista
ORDER BY ciclista DESC;
SELECT equip , AVG(`edat`) AS Media_ed, max(edat) AS Max_ed, min(edat) AS Min_ed , count(*) AS Cant_cic
FROM ciclistes
GROUP BY equip;
SELECT equip , AVG(`edat`) AS Media_ed, max(edat) AS Max_ed, min(edat) AS Min_ed , count(*)
FROM ciclistes
GROUP BY equip;
SELECT nom , AVG(`edat`) AS Media_ed, max(edat) AS Max_ed, min(edat) AS Min_ed , count(*)
FROM ciclistes
GROUP BY equip;
SELECT nom , AVG(`edat`) AS Media_ed, max(edat) AS Max_ed, min(edat) AS Min_ed , count(*)
FROM equips
GROUP BY nom;
CREATE TABLE premis
SELECT color AS color_maillot, premi AS valor_premi
FROM mallots;
CREATE TABLE etapes_ciutat
SELECT `numero`
FROM etapes
WHERE arribada = eixida;
CREATE TABLE etapes_ciutat
SELECT `numero`
FROM etapes
WHERE arriba = eixida;
...
SELECT sum(kms) AS TOTAL_VUElTA
FROM etapes;
SELECT MAX(altura), MIN(altura)
FROM ports;
SELECT max(altura) min(altura)
FROM ports;
SELECT max(alutra) min(altura)
FROM ports;
SELECT AVG(edat)
FROM ciclistes;
SELECT count(*)
